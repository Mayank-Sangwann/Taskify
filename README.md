
# React Native Todo App

This is a To-Do application developed in React Native using Expo and SQLite.


## Features

- **Task Management**: Create, view, edit, and delete tasks.
- **Categorization**: Organize tasks into different categories.
- **User-Friendly Interface**: Clean and minimal design for easy navigation.

## App Demo


<p align="center">
  <img src="https://github.com/user-attachments/assets/7afeb4e1-66c6-4f97-bb8e-a9c4a5f1b8bb" alt="Image 1" width="200" />
  <img src="https://github.com/user-attachments/assets/7a5ca42b-c777-4fbc-a46c-70ece4976f13" alt="Image 1" width="200" />
  <img src="https://github.com/user-attachments/assets/1562a641-5420-4483-bef8-1ce10c37b6e9" alt="Image 2" width="200" />
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/77627f9c-6f36-45b3-ad54-fcad15a1900d" alt="Image 3" width="200" />
  <img src="https://github.com/user-attachments/assets/6b206c1a-73d3-4b39-8200-966165236713" alt="Image 4" width="200" />
  <img src="https://github.com/user-attachments/assets/c10ca228-7f3f-4066-87ca-ac645e4b0831" alt="Image 5" width="200" />
</p>

## Design Choices

- **Technology Stack**: React Native, Expo, SQLite for local storage.
- **User Interface Design**: Minimalist approach, characterized by clean lines and ample white space, complemented by bold, thick fonts that enhance readability and make a striking visual impact while focusing on maximum usability.

## Challenges Encountered

- **Dependency Management**: Managing and updating dependencies, especially in a rapidly evolving ecosystem, sometimes led to compatibility issues and required significant time to resolve.
- **Database Integration**: Integrating SQLite for local data storage was the most challenging part which required careful planning, especially in handling asynchronous operations.


## Installation

1. Clone the repository.
2. Run `npm install`.
3. Start the application.

